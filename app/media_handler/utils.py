...
from app.media_handler.schemas import CarAttributes
...
async def save_file_websocket_connection(file):
    ...
    filepath = f"app/media/{unique_code}/{file}"

    os.makedirs(os.path.dirname(filepath), exist_ok=True)
    url: str = str(filepath.replace("app/", ""))
...
async def save_binary(file_base64: str, filename: str) -> Tuple[str, str]:
    ...
    return: A tuple containing the URL and filepath of the saved file. The URL is a relative path excluding the "app/" prefix.
    """
    unique_code = get_random_string(length=6)
    filepath = f"app/media/{unique_code}_{filename}"
...
async def save_file(file) -> Tuple[str, str]:
    ...
    """
    unique_code = get_random_string(length=6)
    filepath = f"app/media/{unique_code}_{file.filename}"
    os.makedirs(os.path.dirname(filepath), exist_ok=True)
    url: str = str(filepath.replace("app/", ""))
